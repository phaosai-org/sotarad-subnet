...
**Documented baseline (example, not mandatory):** `[0llheaven/Llama-3.2-11B-Vision-Radiology-mini](https://huggingface.co/0llheaven/Llama-3.2-11B-Vision-Radiology-mini)` on Hugging Face. It is a **Llama-3.2-11B-Vision**–family model (image-text-to-text), fine-tuned from an **Instruct** 4-bit variant (~**11B parameters** per the model card), with naming and lineage that indicate **radiology-oriented** adaptation. Use that page for exact files, license (Apache-2.0 on the card), and revision history.
